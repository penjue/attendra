...
function App() {
  ...
  const [activationLink, setActivationLink] = useState<{ deviceId: string; deviceName: string; url: string } | null>(null);
  ...
  const deleteBranch = async (branch: Branch) => { if (!window.confirm(`Permanently delete branch "${branch.name}"? This is only allowed when it has no tablets, shifts or attendance history.`)) return; setBranchMessage(''); try { await fetchJson(`${API_URL}/v1/admin/branches/${branch.id}`, { method: 'DELETE' }); setBranchMessage('Branch deleted permanently.'); await Promise.all([loadBranches(), loadDevices(), loadOverview()]); } catch (error: any) { setBranchMessage(error.message==='BRANCH_HAS_HISTORY'?'Protected record: this branch has tablets, shifts or attendance history and cannot be permanently deleted. Deactivate it instead.':error.message==='BRANCH_DELETE_FAILED'?'Attendra could not safely delete this branch, so it has been preserved. Deactivate it instead or review its linked records.':'Unable to delete this branch.'); } };
  ...
  const deleteDevice = async (device: Device) => { if (!window.confirm(`Permanently delete tablet "${device.name}"? This is only allowed when it has no attendance history.`)) return; setDeviceMessage(''); try { await fetchJson(`${API_URL}/v1/admin/devices/${device.id}`, { method: 'DELETE' }); setDeviceMessage('Tablet deleted permanently.'); await Promise.all([loadDevices(), loadBranches(), loadOverview()]); } catch (error: any) { setDeviceMessage(error.message==='DEVICE_HAS_HISTORY'?'Protected record: this tablet has attendance history and cannot be permanently deleted. Deactivate it instead.':error.message==='DEVICE_DELETE_FAILED'?'Attendra could not safely delete this tablet, so it has been preserved. Deactivate it instead or review its linked records.':'Unable to delete this tablet.'); } };

  const generateActivationLink = async (device: Device) => {
    if (!device.active) return setDeviceMessage('Activate this tablet record before generating an activation link.');
    setDeviceMessage(''); setActivationLink(null); setActivationQr('');
    try {
      const data = await fetchJson(`${API_URL}/v1/admin/devices/${device.id}/activation-token`, { method: 'POST', body: '{}' });
      const params = new URLSearchParams({ activationToken: data.activationToken });
      const tabletUrl = env.VITE_TABLET_URL ?? 'https://attendra-tablet.onrender.com';
      const url = `${tabletUrl}/?${params.toString()}`;
      setActivationLink({ deviceId: device.id, deviceName: device.name, url });
      setActivationQr(await QRCode.toDataURL(url, { width: 280, margin: 2 }));
      setDeviceMessage('One-time tablet activation generated. It expires in 10 minutes and can only be used once.');
    } catch { setDeviceMessage('Unable to generate the tablet activation link. Please try again.'); }
  };
  const copyActivationLink = async () => {
    if (!activationLink) return;
    try { await navigator.clipboard.writeText(activationLink.url); setDeviceMessage('Activation link copied. Open it only on the intended tablet.'); }
    catch { setDeviceMessage('Copy was blocked by this browser. Select the activation link below and copy it manually.'); }
  };
  ...
  const exportTimeReportCsv = () => {
    if (!timeRows.length) return setTimeMessage('There is no report data to export for this period.');
    const header = ['Employee number','Employee name','Worked minutes','Scheduled minutes','Overtime minutes','Late check-ins','Early check-ins','Missed shifts','Open session'];
    const rows = timeRows.map(row => [row.employeeNumber,row.employeeName,row.workedMinutes,row.scheduledMinutes,includeOvertime?row.overtimeMinutes:0,row.lateEvents,row.earlyEvents,row.missedShifts,row.openSession?'Yes':'No']);
    const csv = [header,...rows].map(row=>row.map(csvCell).join(',')).join('\\r\\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8' });
    const url = URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=`attendra-workforce-report-${timeFrom}-to-${timeTo}.csv`; a.click(); URL.revokeObjectURL(url);
  ...
  return <main className="shell">
    ...
    {activeTab === 'reports' && <>
      <section className="grid"><article><strong>{minutesLabel(timeTotals.workedMinutes)}</strong><span>Worked</span></article><article><strong>{minutesLabel(timeTotals.scheduledMinutes)}</strong><span>Scheduled</span></article><article><strong>{includeOvertime?minutesLabel(timeTotals.overtimeMinutes):'Excluded'}</strong><span>Overtime</span></article><article><strong>{timeTotals.missedShifts}</strong><span>Missed shifts</span></article></section>
      <section className="panel"><div className="panelHead"><div><h2>Workforce report</h2><span>Manager summary for payroll preparation and attendance review.</span></div><div className="rowActions"><button className="primary" onClick={exportTimeReportCsv}>Export CSV</button></div></div>
        <div className="reportTools"><label>From<input type="date" value={timeFrom} max={timeTo} onChange={e=>setTimeFrom(e.target.value)} /></label><label>To<input type="date" value={timeTo} min={timeFrom} onChange={e=>setTimeTo(e.target.value)} /></label><button className="primary" onClick={loadTimeReport}>Apply period</button></div>
        {timeMessage&&<div className="infoBox">{timeMessage}</div>}
        <div className="directoryTools"><input value={timeSearch} onChange={e=>setTimeSearch(e.target.value)} placeholder="Search employee" /></div>
        <div className="branchList">{filteredTimeRows.length===0?<p className="empty">No report data for this period.</p>:filteredTimeRows.map(row=><div className="branchRow" key={row.employeeId}><div className="branchSummary"><strong>{row.employeeName}</strong><span>#{row.employeeNumber}</span><small>Worked {minutesLabel(row.workedMinutes)} · Scheduled {minutesLabel(row.scheduledMinutes)} · {includeOvertime?`Overtime ${minutesLabel(row.overtimeMinutes)}`:'Overtime excluded'}</small><small>{row.lateEvents} late · {row.missedShifts} missed · {row.openSession?'Open session':'Complete sessions'}</small></div></div>)}</div>
        <p className="formHint">CSV exports the selected company report period and follows the current overtime inclusion setting.</p>
      </section>
    </>}
