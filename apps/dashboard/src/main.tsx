...
function App() {
  ...
  const deleteDevice = async (device: Device) => { if (!window.confirm(`Permanently delete tablet "${device.name}"? This is only allowed when it has no attendance history.`)) return; setDeviceMessage(''); try { await fetchJson(`${API_URL}/v1/admin/devices/${device.id}`, { method: 'DELETE' }); setDeviceMessage('Tablet deleted permanently.'); await Promise.all([loadDevices(), loadBranches(), loadOverview()]); } catch (error: any) { setDeviceMessage(error.message==='DEVICE_HAS_HISTORY'?'This tablet has attendance history and cannot be permanently deleted. Deactivate it instead.':'Unable to delete this tablet.'); } };
